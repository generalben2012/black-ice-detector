# Black Ice Detector

Arduino UNO Q와 각종 센서를 사용하여 블랙아이스를 감지하고 웹 인터페이스에 표시하는 프로젝트입니다.

## 프로젝트 개요

이 프로젝트는 HC-SR04 초음파 센서와 LDR 조도 센서를 사용하여 도로 표면의 Black Ice를 감지하고, Arduino UNO Q의 Bridge 기능을 통해 Python 백엔드로 데이터를 전송하며, 웹 인터페이스를 통해 블랙아이스 감지 상태를 실시간으로 표시합니다.  
또한 **별도의 Arduino 보드에서 DHT22로 온습도를 측정**하고, LCD에 표시된 값을 **웹 수동 입력**에 반영합니다.

### 주요 기능

- **블랙아이스 감지**: HC-SR04 초음파 센서를 사용하여 도로 표면과의 거리를 측정하여 블랙아이스 감지
- **조도 측정**: LDR 조도 센서를 사용하여 주변 밝기 측정
- **웹 인터페이스**: 실시간으로 거리 값과 조도 값을 웹 브라우저에 표시
- **자동 업데이트**: 100ms마다 센서 값 자동 업데이트 (초당 10회)
- **연결 상태 표시**: 센서 연결 상태 및 측정 상태를 시각적으로 표시
- **외부 온습도 보드 연동**: 별도 Arduino+DHT22+LCD에서 온습도를 측정하고 웹에 수동 입력

## 하드웨어 요구사항

- **Arduino UNO Q** (x1)
- **HC-SR04 초음파 센서** (x1)
- **LDR 조도 센서** (x1)
- **470Ω 저항** (x1) - LDR 분압 회로용
- **점퍼 와이어** (x6)
- **USB-C 케이블** (전원 및 프로그래밍용)
- **Arduino 보드 (UNO/MEGA 등)** (x1) - 외부 온습도 전용
- **DHT22 온습도 센서** (x1)
- **I2C 16x2 LCD** (x1, 0x27 또는 0x3F)
- **USB 케이블** (외부 Arduino 전원/시리얼 확인용)

## 소프트웨어 요구사항

- Arduino App Lab

## 센서 연결 방법

### HC-SR04 초음파 센서 연결

HC-SR04 초음파 센서를 Arduino UNO Q에 다음과 같이 연결합니다:

| HC-SR04 핀 | Arduino UNO Q 핀 | 설명        |
|------------|-----------------|-------------|
| VCC        | 5V              | 전원 공급   |
| GND        | GND             | 그라운드    |
| Trig       | D2              | 트리거 신호 (출력) |
| Echo       | D3              | 에코 신호 (입력)  |

### LDR 조도 센서 연결

LDR 조도 센서는 분압 회로(voltage divider)로 구성합니다:

**회로 구성:**
```
5V ──[LDR]──┬──[470Ω 저항]── GND
             │
            A1
```

**연결 방법:**
- LDR의 한쪽을 5V에 연결
- LDR의 다른 쪽을 470Ω 저항과 연결하고, 그 중간점을 A1에 연결
- 470Ω 저항의 다른 쪽을 GND에 연결



### 연결 다이어그램

```
HC-SR04                    Arduino UNO Q
┌─────────┐                ┌──────────┐
│  VCC    │───────────────▶│   5V     │
│  GND    │───────────────▶│   GND    │
│  Trig   │───────────────▶│   D2     │
│  Echo   │───────────────▶│   D3     │
└─────────┘                └──────────┘
```

## 사용 방법

1. **하드웨어 연결**
   - 위의 연결 방법에 따라 HC-SR04 센서를 Arduino UNO Q에 연결합니다.

2. **앱 실행**
   - Arduino App Lab에서 이 프로젝트를 엽니다.
   - "Run App" 버튼을 클릭하여 앱을 실행합니다.

3. **웹 인터페이스 접속**
   - 앱이 자동으로 웹 브라우저를 엽니다.
   - 또는 수동으로 `<board-name>.local:7000`으로 접속할 수 있습니다.

4. **센서 값 확인**
   - 센서 앞에 물체를 두면 실시간으로 거리가 표시됩니다.
   - 거리는 센티미터(cm) 단위로 표시됩니다.
   - 조도 센서 값은 0~1023 범위로 표시됩니다.
5. **외부 온습도 보드 값 입력**
   - 외부 Arduino의 LCD에 표시되는 온도/습도를 확인합니다.
   - 웹 화면의 **수동 입력(온도/습도)** 칸에 그대로 입력합니다.

## 프로젝트 구조

```
black-ice-detector/
├── app.yaml              # 앱 설정 파일
├── python/
│   └── main.py          # Python 백엔드 (Bridge 통신)
├── sketch/
│   └── sketch.ino       # Arduino 스케치 (센서 제어)
├── assets/
│   ├── index.html        # 웹 인터페이스 HTML
│   ├── app.js           # 웹 인터페이스 JavaScript
│   ├── style.css        # 웹 인터페이스 스타일
│   └── libs/
│       └── socket.io.min.js  # WebSocket 라이브러리
└── README.md            # 프로젝트 설명서
```

## 작동 원리

### 1. Arduino 스케치 (`sketch.ino`)

Arduino 스케치는 HC-SR04 센서와 LDR 조도 센서를 제어하고 값을 측정합니다:

- **HC-SR04 센서**:
  - 트리거 신호: D2 핀으로 10마이크로초 펄스를 보냅니다.
  - 에코 신호: D3 핀에서 반사된 신호의 시간을 측정합니다.
  - 거리 계산: `거리 = (시간 × 음속) / 2` 공식을 사용하여 거리를 계산합니다.
- **LDR 조도 센서**:
  - A1 핀에서 아날로그 값을 읽어 조도를 측정합니다 (0~1023).
- **Bridge 통신**: Python 백엔드에서 호출할 수 있도록 다음 함수들을 제공합니다:
  - `get_duration()`: 펄스 지속 시간 반환
  - `get_distance_mm()`: 거리 값 반환 (mm 단위)
  - `get_ldr_value()`: 조도 센서 값 반환

### 1-1. 외부 온습도 보드 (DHT22 + LCD)

- **별도 Arduino 보드**에서 DHT22를 읽어 **LCD에 표시**
- DHT22 값은 **웹 수동 입력**에 반영
- 웹 측정 요청 시 **온도/습도 값이 함께 저장**

```text
[외부 Arduino + DHT22]
  └─ LCD 표시(온도/습도)
        └─ 사용자가 웹 입력 폼에 수동 입력
```

### 2. Python 백엔드 (`main.py`)

Python 백엔드는 Bridge를 통해 Arduino에서 센서 값을 읽고 웹 클라이언트에 전송합니다:

- **Bridge 통신**: `Bridge.call()`을 사용하여 Arduino에서 거리 값과 조도 값을 읽습니다.
- **WebSocket 통신**: `WebUI` Brick을 사용하여 웹 클라이언트와 실시간 통신합니다.
- **자동 업데이트**: 100ms마다 센서 값을 읽어 웹 클라이언트에 전송합니다.
- **측정 결과 저장**: 수동 입력된 온도/습도 값을 함께 기록합니다.

### 3. 웹 인터페이스 (`index.html`, `app.js`, `style.css`)

웹 인터페이스는 WebSocket을 통해 실시간으로 센서 값을 받아 표시합니다:

- **WebSocket 연결**: Socket.IO를 사용하여 Python 백엔드에 연결합니다.
- **실시간 업데이트**: `distance_update` 이벤트를 받아 거리 값과 조도 값을 업데이트합니다.
- **상태 표시**: 연결 상태 및 측정 상태를 시각적으로 표시합니다.
- **수동 입력**: 외부 온습도 보드 값을 입력하여 측정 요청에 포함합니다.

## 전체 데이터 흐름 (그림)

```mermaid
flowchart LR
    A[UNO Q + HC-SR04 + LDR] -->|Bridge| B[Python 백엔드]
    C[외부 Arduino + DHT22 + LCD] -->|사람이 읽음| D[웹 수동 입력]
    D -->|측정 요청| B
    B -->|WebSocket| E[웹 UI]
    B -->|JSONL 저장| F[measurements.jsonl]
```

## 커스터마이징

### 측정 범위 변경

`sketch.ino` 파일에서 다음 상수를 수정할 수 있습니다:

```cpp
const float MAX_DISTANCE = 400.0;  // 최대 측정 거리 (cm)
const float MIN_DISTANCE = 2.0;     // 최소 측정 거리 (cm)
```

### 업데이트 주기 변경

`python/main.py` 파일에서 다음 상수를 수정할 수 있습니다:

```python
UPDATE_INTERVAL = 0.1  # 업데이트 주기 (초)
```

### 핀 번호 변경

`sketch.ino` 파일에서 다음 상수를 수정할 수 있습니다:

```cpp
#define TRIG_PIN 2   // 트리거 핀
#define ECHO_PIN 3   // 에코 핀
#define LDR_PIN A1   // 조도 센서 핀
```

## 문제 해결

### 거리 값이 표시되지 않음

1. 센서 연결 확인: VCC, GND, Trig, Echo 핀이 올바르게 연결되었는지 확인합니다.
2. 전원 확인: 센서에 5V 전원이 공급되고 있는지 확인합니다.
3. 센서 위치 확인: 센서 앞에 물체가 2cm ~ 400cm 범위 내에 있는지 확인합니다.

### 연결 오류

1. Arduino UNO Q 연결 확인: USB 케이블이 올바르게 연결되었는지 확인합니다.
2. 앱 재시작: Arduino App Lab에서 앱을 재시작합니다.
3. 브라우저 콘솔 확인: 브라우저 개발자 도구의 콘솔에서 오류 메시지를 확인합니다.

### 측정 값이 부정확함

1. 센서 정렬 확인: 센서가 물체를 정확히 향하고 있는지 확인합니다.
2. 환경 확인: 센서 주변에 반사되는 물체가 없는지 확인합니다.
3. 센서 교체: 센서가 손상되었을 수 있으므로 다른 센서로 교체해봅니다.

## 참고 자료

- [Arduino UNO Q 공식 문서](https://docs.arduino.cc/hardware/uno-q)
- [HC-SR04 초음파 센서 데이터시트](https://cdn.sparkfun.com/datasheets/Sensors/Proximity/HCSR04.pdf)
- [Arduino App Lab 문서](https://docs.arduino.cc/arduino-cloud/app-lab)

## 라이선스

SPDX-License-Identifier: MPL-2.0

